...
(function($)
{
	...
	$(document).on('ready', function () {
		...
		$('.menu__link , .era_li_footer').on('click', function(){
			var who;
			if ($(this).hasClass('era')){
				...
				$([document.documentElement, document.body]).animate({
					scrollTop: $("#era_elem_" + elemNumber).offset().top - who
				}, 500);
			}
		});
		
		$('.video_era_0').children(".overlay_video").css("opacity" , "1");
		$('.photostack_figure_0').children(".video_disc_era").css("opacity" , "1");
		$('.era_circ').on('click', function(){
			var elemNumber_fihure = $(this).attr('id').substr(-1);
			$('.overlay_video').css({
				"opacity" : "0",
			});
			$('.video_disc_era').css("opacity" , "0");
			$('.video_era_' + elemNumber_fihure).children(".overlay_video").css("opacity" , "1");
			$('.photostack_figure_' + elemNumber_fihure).children(".video_disc_era").css("opacity" , "1");
			//console.log($('.video_era_' + elemNumber + ' .overlay_video'));
		});
